...
x_window_r, y_window_r = 200, 280
# Размеры малых(верхних) окон
x_window_s, y_window_s = 80, 70
# Размеры больших(нижних) окон
x_window_b, y_window_b = 80, 170
# Координаты окон
xw_1, yw_1 = 290, 10
xw_2, yw_2 = 60, 10
xw_3, yw_3 = -170, 10
...
def window(x, y, scale):
    ...
    """
    # Внешняя рама окна
    rect(screen, big_window_colour, (x, y, x_window_r * scale, y_window_r * scale), 0)
    # левое верхнее малое окно
    rect(screen, small_window_colour, (x + x_window_r * scale // 20, y + y_window_r // 28,
                                       x_window_s, y_window_s), 0)
    # правое верхнее малое окно
    rect(screen, small_window_colour, (x + x_window_r * scale * 2 // 20 + x_window_s, y + y_window_r // 28,
                                       x_window_s, y_window_s), 0)
    # левое нижнее малое окно
    rect(screen, small_window_colour, (x + x_window_r * scale // 20, y + y_window_r * 3 // 28 + y_window_s,
                                       x_window_b, y_window_b), 0)
    # правое нижнее малое окно
    rect(screen, small_window_colour, (x + x_window_r * scale * 2 // 20 + x_window_s,
...
def cat_left_tail(x, y, k, cat_colour):
    ...
    :param x: x-координата верхнего левого угла прямоугольника, содержащего тело кошки
    :param y: y-координата верхнего левого угла прямоугольника, содержащего тело кошки
    :param k: масштаб
...
def cat_left_back_leg(x, y, k, cat_colour):
    ...
    circle(screen, black, (int(x + 270 * k), int(y + 130 * k)), int(50 * k), 1)
    ellipse(screen, cat_colour, (x + 290 * k, y + 130 * k, 50 * k, 100 * k), 0)
    ellipse(screen, black, (x + 290 * k, y + 130 * k, 50 * k, 100 * k), 1)
...
def cat_left_right_ear(x, y, k, cat_colour):
    ...
    polygon(screen, cat_colour,
            ((int(x + 62 * k), int(y + 2 * k)), (int(x + 32 * k), int(y + 23 * k)),
             (int(x + 63 * k), int(y + 43 * k))))
    polygon(screen, black,
            ((int(x + 62 * k), int(y + 2 * k)), (int(x + 32 * k), int(y + 23 * k)),
             (int(x + 63 * k), int(y + 43 * k))), 1)
    polygon(screen, pink,
            ((int(x + 58 * k), int(y + 7 * k)), (int(x + 37 * k), int(y + 22 * k)),
...
def cat_left_left_ear(x, y, k, cat_colour):
    ...
    """
    polygon(screen, cat_colour,
            ((int(x - 52 * k), int(y + 2 * k)), (int(x - 22 * k), int(y + 23 * k)),
    ...
    polygon(screen, black,
            ...
            1)
    polygon(screen, pink,
            ((int(x - 48 * k), int(y + 7 * k)), (int(x - 27 * k), int(y + 22 * k)),
...
def cat_left_nose(x, y, k):
    ...
    """
    polygon(screen, pink,
            ((int(x + 10 * k), int(y + 100 * k)), (int(x + 1 * k), int(y + 95 * k)),
...
def cat_left_mouth(x, y, k):
    ...
    :return:
    """
    arc(screen, black, (x + 10 * k, y + 80 * k, 40 * k, 40 * k), pi, 3 * pi / 2, 1)
    arc(screen, black, (x - 30 * k, y + 80 * k, 40 * k, 40 * k), 3 * pi / 2, 2 * pi, 1)
...
def cat_left_whiskers(x, y, k):
    ...
    :return:
    """
    arc(screen, black, (x - 55 * k, y + 95 * k, 150 * k, 40 * k), 0.2, pi / 2, 1)
    arc(screen, black, (x - 45 * k, y + 98 * k, 130 * k, 40 * k), 0.2, pi / 2, 1)
    arc(screen, black, (x - 35 * k, y + 101 * k, 110 * k, 40 * k), 0.2, pi / 2, 1)
    arc(screen, black, (x - 80 * k, y + 95 * k, 150 * k, 40 * k),  pi / 2, pi - 0.2, 1)
    arc(screen, black, (x - 70 * k, y + 98 * k, 130 * k, 40 * k), pi / 2, pi - 0.2, 1)
    arc(screen, black, (x - 60 * k, y + 101 * k, 110 * k, 40 * k), pi / 2, pi - 0.2, 1)
...
def cat_right_back_leg(x, y, k, cat_colour):
    ...
    circle(screen, black, (int(x + 50 * k), int(y + 130 * k)), int(50 * k), 1)
...
def cat_right_head(x, y, k, cat_colour):
    ...
    circle(screen, black, (int(x + 320 * k), int(y + 75 * k)), int(70 * k), 1)
...
def cat_right_eyes(x, y, k, eyes_colour):
    ...
    circle(screen, black, (int(x + 350 * k), int(y + 70 * k)), int(18 * k), 1)
    circle(screen, black, (int(x + 290 * k), int(y + 70 * k)), int(18 * k), 1)
...
def cat_right_nose(x, y, k):
    ...
    """
    polygon(screen, pink,
            ((int(x + 320 * k), int(y + 100 * k)), (int(x + 311 * k), int(y + 95 * k)),
...
def cat_right_mouth(x, y, k):
    ...
    :return:
    """
    arc(screen, black, (x + 320 * k, y + 80 * k, 40 * k, 40 * k), pi, 3 * pi / 2, 1)
    arc(screen, black, (x + 280 * k, y + 80 * k, 40 * k, 40 * k), 3 * pi / 2, 2 * pi, 1)
...
def cat_right_whiskers(x, y, k):
    ...
    :return:
    """
    arc(screen, black, (x + 255 * k, y + 95 * k, 150 * k, 40 * k), 0.2, pi / 2, 1)
    arc(screen, black, (x + 265 * k, y + 98 * k, 130 * k, 40 * k), 0.2, pi / 2, 1)
    arc(screen, black, (x + 275 * k, y + 101 * k, 110 * k, 40 * k), 0.2, pi / 2, 1)
    arc(screen, black, (x + 230 * k, y + 95 * k, 150 * k, 40 * k), pi / 2, pi - 0.2, 1)
    arc(screen, black, (x + 240 * k, y + 98 * k, 130 * k, 40 * k), pi / 2, pi - 0.2, 1)
    arc(screen, black, (x + 250 * k, y + 101 * k, 110 * k, 40 * k), pi / 2, pi - 0.2, 1)
...
def cat_right_right_ear(x, y, k, cat_colour):
    ...
    """
    polygon(screen, cat_colour,
            ((int(x + 372 * k), int(y + 2 * k)), (int(x + 342 * k), int(y + 23 * k)),
             (int(x + 373 * k), int(y + 43 * k))))
    polygon(screen, black,
            ((int(x + 372 * k), int(y + 2 * k)), (int(x + 342 * k), int(y + 23 * k)),
             (int(x + 373 * k), int(y + 43 * k))), 1)
    polygon(screen, pink,
            ((int(x + 368 * k), int(y + 7 * k)), (int(x + 347 * k), int(y + 22 * k)),
...
def cat_right_left_ear(x, y, k, cat_colour):
    ...
    """
    polygon(screen, cat_colour,
            ((int(x + 258 * k), int(y + 2 * k)), (int(x + 288 * k), int(y + 23 * k)),
    ...
    polygon(screen, black,
            ...
            1)
    polygon(screen, pink,
            ((int(x + 262 * k), int(y + 7 * k)), (int(x + 283 * k), int(y + 22 * k)),
...
def clew_right(x, y, k):
    """
    функция рисует клубок, ориентированный в правую сторону, заданного масштаба
    ...
    arc(screen, black, (x - 85 * k, y - 30 * k, 120 * k, 100 * k), 0, pi / 2, 1)
    arc(screen, black, (x - 80 * k, y - 40 * k, 120 * k, 100 * k), 0, pi / 2, 1)
    arc(screen, black, (x - 20 * k, y - 30 * k, 120 * k, 120 * k), 3 * pi / 4,  pi, 1)
    arc(screen, black, (x - 10 * k, y - 20 * k, 120 * k, 120 * k), 3 * pi / 4, pi, 1)
    arc(screen, black, (x - 5 * k, y - 16 * k, 120 * k, 120 * k), 3 * pi / 4, pi, 1)


def clew_left(x, y, k):
    """
    функция рисует клубок, ориентированный в левую сторону, заданного масштаба
    :param x: x-координата центра клубка
    :param y: y-координата центра клубка
    :param k: масштаб
    :return:
    ...
    circle(screen, grey,  (x, y), int(50 * k), 0)
    circle(screen, black, (x, y), int(50 * k), 1)
    arc(screen, black, (x - 40 * k, y - 30 * k, 120 * k, 100 * k), pi / 2, pi, 1)
    arc(screen, black, (x - 40 * k, y - 40 * k, 120 * k, 100 * k), pi / 2, pi,  1)
    arc(screen, black, (x - 90 * k, y - 30 * k, 120 * k, 120 * k), 0,  pi / 4, 1)
    arc(screen, black, (x - 95 * k, y - 20 * k, 120 * k, 120 * k), 0,  pi / 4, 1)
    arc(screen, black, (x - 100 * k, y - 16 * k, 120 * k, 120 * k), 0,  pi / 4, 1)


# Основная программа
background()
window(xw_1, yw_1, scale0)
window(xw_2, yw_2, scale0)
window(xw_3, yw_3, scale0)
